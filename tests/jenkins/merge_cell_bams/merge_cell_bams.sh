...
docker run -w $PWD -v $PWD:$PWD -v /refdata:/refdata -v /var/run/docker.sock:/var/run/docker.sock \
  ...
  for f in MERGE_CELL_BAMS/ref_test_data/testdata/*.bam; do samtools view -f 4 -c $f; done > MERGE_CELL_BAMS/output/unmapped;
  for f in MERGE_CELL_BAMS/ref_test_data/testdata/*.bam; do samtools view -c $f; done > MERGE_CELL_BAMS/output/mapped;
| lsMERGE_CELL_BAMS/ref_test_data/testdata/*.bam | awk '{split($0, r, "."); print r[1] }' > MERGE_CELL_BAMS/output/regions
#
#docker run -w $PWD -v $PWD:$PWD -v /refdata:/refdata -v /var/run/docker.sock:/var/run/docker.sock \
#  -v /usr/bin/docker:/usr/bin/docker --rm \
#  $3/single_cell_pipeline:$TAG \
